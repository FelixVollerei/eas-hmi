> Historical EAS-HMI documentation. The current release is [EAS Visual](../README.md). Commands and measurements below belong to the earlier HMI release.
...
当前结果见 [复核报告](review-corrections.md)；[阶段 6 报告](stage-6-report.md)保留原交付时的测量。
...
[JSON Schema](../schemas/project.schema.json) 由 Pydantic 模型生成；[设计约定](design-contract.md) 说明坐标、ID 和同步范围。
...
所有数据均为合成，`--` 数值是设计态占位。见 [Demo 详细说明](stage-5-demo.md)。
...
[完整 CLI 约定](stage-4-cli.md) 和各命令 `--help` 提供参数及限制。
...
成功证据由 `build/stage6/latest.json` 指向；完整流程见 [最终复验说明](final-verification-guide.md)。
...
- [最终验收报告](stage-6-report.md)
- [最终逐项验收表](final-acceptance.md)
- [原始要求](requirements-original.txt)
- [六阶段计划](implementation-plan.md) 与 [需求映射](acceptance-matrix.md)
- 历史报告：[阶段 1](stage-1-report.md)、[阶段 2](stage-2-report.md)、[阶段 3](stage-3-report.md)、[阶段 4](stage-4-report.md)、[阶段 5](stage-5-report.md)

## 交付后复核

长历史读放大、INFO、命令/help 与文档更正见 [复核报告](review-corrections.md)。`history --audit` 显式校验全部历史提交，普通历史读取复用经过摘要校验的投影；空事件读取只校验当前提交。所有命令默认 JSON（diff 默认文本、watch 为 JSONL）；`--json` 是保留的显式兼容标志，通过公共包装器统一处理。

未来手动比较使用 [实验题目与流程](agent-comparison/protocol.md)、[CLI 组 prompt](agent-comparison/prompt-semantic.md)、[视觉组 prompt](agent-comparison/prompt-visual.md)。本次没有执行 Agent 对照实验。原生崩溃仍未归因，见 [诊断说明](native-crash-diagnostics.md)。
